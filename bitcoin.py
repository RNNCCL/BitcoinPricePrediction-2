...
def computeDelta(wt, X, Xi):
    ...
    #print len(Xi), len(X), wt
    Yivalues = Xi['Yi'].tolist()
    Xi = Xi.drop('Yi', 1)
    Xivalues= Xi.values.tolist()
    Xvalues = X[:-1]
    
    numerator = 0.0
    denominator = 0.0
    mean1 = np.mean(Xvalues)
    for i in range(0,len(Xivalues)):
	mean2 = np.mean(Xivalues[i])
	summation = 0.0
	summation = np.sum((Xvalues - mean1) * (Xivalues[i] - mean2))
	similarity = summation/(len(Xvalues) * np.std(Xvalues) * np.std(Xivalues[i]))

	numerator = numerator + Yivalues[i] * math.exp(wt * similarity)
	denominator = denominator + math.exp(wt * similarity)
